...
const actionUrl = computed(() => {
  if (props.attachment) {
    return props.attachment.previewUrl || undefined
  }
  if (props.messageAttachment?.url) {
    return `/api/files/preview/${props.messageAttachment.url.replace(/^\//, '')}`
  }
  return undefined
})

const actionIcon = computed(() => isImage.value ? 'i-lucide-eye' : 'i-lucide-download')

const { visibleRef, imgsRef, indexRef, show } = useEasyLightbox({
  imgs: [],
})
  ...
  <div>
    ...
    >
      ...
      <div
        ...
        class="hover-mask absolute inset-0 bg-black/30 opacity-0 transition-opacity z-10 flex items-center justify-center"
      >
        <button
          v-if="actionUrl"
          class="w-8 h-8 rounded-full bg-black/50 hover:bg-black/70 text-white flex items-center justify-center transition-colors cursor-pointer"
