...
- [x] 会话列表按业务隔离（仅展示与当前 business_key 相关的记录，默认展示 "default"）
- [x] 会话搜索（在侧边栏点击搜索按钮或 Cmd/Ctrl + K 打开搜索弹窗，输入标题关键词检索当前业务下的会话）
- [x] 停止生成（流式过程中可中断，调用后端停止 API 并显示专用停止消息块）
...
- [x] 滚动到最新位置（生成时若位于底部则自动滚动，否则保持当前位置）
- [x] 虚拟滚动支持（基于 @tanstack/vue-virtual，优化超长对话渲染性能）
...
- [x] 语言设置集成（外观 > 语言分组）
- [x] URL 参数支持（lang=?）
- [x] 100% 文本覆盖（包括 Nuxt UI 组件库文本）
- [x] 日期本地化（dayjs）
- [x] 语言切换无需刷新
- [x] Nuxt UI 组件库国际化集成（UChatPrompt 等组件自动切换语言）
